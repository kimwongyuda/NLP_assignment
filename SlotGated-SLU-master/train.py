...
parser.add_argument("--intent_file_one", type=str, default='label_one', help="Intent_one file name.")
parser.add_argument("--intent_file_two", type=str, default='label_two', help="Intent_two file name.")
...
createVocabulary(os.path.join(full_train_path, arg.intent_file_one), os.path.join(arg.vocab_path, 'intent_one_vocab'))
createVocabulary(os.path.join(full_train_path, arg.intent_file_two), os.path.join(arg.vocab_path, 'intent_two_vocab'))
...
intent_one_vocab = loadVocabulary(os.path.join(arg.vocab_path, 'intent_one_vocab'))
intent_two_vocab = loadVocabulary(os.path.join(arg.vocab_path, 'intent_two_vocab'))


#바뀐 부분
def createModel(input_data, input_size, sequence_length, slot_size, intent_size,intent_size_2, layer_size = 128, isTraining = True):
    ...
    print()
    print('state_outputs: ', state_outputs)
    print()
    print('final_state: ', final_state)
    print()
#     print('state_outputs: ',state_outputs.eval())
#     print('final_state: ',final_state.eval())

    #bidirectional 이기 때문에..
    ...
    print('state_outputs 2: ', state_outputs)
    print()
    print('final_state 2: ', final_state)
...
#             intent_gate = rnn_cell_impl._linear(intent_output, attn_size, True)
    ...
    print()
    print('slot_output: ', slot_output)
    print()
    print('intent_output: ', intent_output)
    print()
    # 첫번째 아웃풋
...
#         slot = rnn_cell_impl._linear(slot_output, slot_size, True)
    ...
    print('2번째 아웃풋 내기(state_outputs, final_state): ', state_outputs, final_state)
    
    # 이전 결과 concat
    intent_output_2 = tf.concat([final_state,intent],1)
    print('2번째 아웃풋(linear 전): ', intent_output_2)
    print('intent_size_2의 값: ', intent_size_2)
    with tf.variable_scope('intene_proj_2'):
        intent_2 = core_rnn_cell._linear(intent_output_2, intent_size_2,True)
    print('2번째 linear 후: ', intent_2)    
        
    outputs = [slot, intent, intent_2]
    
    print()
    print('outputs(slot, intent): ', outputs)
    print()
...
input_data = tf.placeholder(tf.int32, [None, None], name='inputs')# 모델에 들어감, 각 문장(인덱스화)
sequence_length = tf.placeholder(tf.int32, [None], name="sequence_length")# 모델에 들어감, 각 문장 길이
...
intent = tf.placeholder(tf.int32, [None], name='intent')#loss 구할 때, true y값
intent_2 = tf.placeholder(tf.int32, [None], name='intent_2')

# 모델 만들기
# intent vocaba을 2개?
# 일단 단어집 3개 들어감
with tf.variable_scope('model'):
    training_outputs = createModel(input_data, len(in_vocab['vocab']), sequence_length, len(slot_vocab['vocab']), len(intent_one_vocab['vocab']), len(intent_two_vocab['vocab']),layer_size=arg.layer_size)
...
# pred_intent 해서 로스 구하기
intent_output = training_outputs[1]
intent_output_2 = training_outputs[2]
with tf.variable_scope('intent_loss'):
    crossent =tf.nn.sparse_softmax_cross_entropy_with_logits(labels=intent, logits=intent_output)
    print('crossent: ', crossent)
    crossent_2 =tf.nn.sparse_softmax_cross_entropy_with_logits(labels=intent_2, logits=intent_output_2)
    print('crossent2: ', crossent_2)
    final_crossent = crossent + crossent_2
    intent_loss = tf.reduce_sum(final_crossent) / tf.cast(arg.batch_size, tf.float32)

#아래는 loss 업데이트 굳이 손 안되도 되어 보임.
...
with tf.variable_scope('model', reuse=True):
    #바뀐 부분
    inference_outputs = createModel(input_data, len(in_vocab['vocab']), sequence_length, len(slot_vocab['vocab']), len(intent_one_vocab['vocab']), len(intent_two_vocab['vocab']) ,layer_size=arg.layer_size, isTraining=False)
...
inference_intent_output_2 = tf.nn.softmax(inference_outputs[2], name='intent_output_2')
    
inference_outputs = [inference_intent_output, inference_slot_output, inference_intent_output_2]
...
with tf.Session() as sess:
    ...
    while True:
        if data_processor == None:
            #바뀐 부분
            data_processor = DataProcessor(os.path.join(full_train_path, arg.input_file), os.path.join(full_train_path, arg.slot_file), os.path.join(full_train_path, arg.intent_file_one), os.path.join(full_train_path, arg.intent_file_two),in_vocab, slot_vocab, intent_one_vocab, intent_two_vocab)
        in_data, slot_data, slot_weight, length, intents, intents_2 ,_,_,_,_ = data_processor.get_batch(arg.batch_size)

        #모델에 넣어 줄것
        feed_dict = {input_data.name: in_data, slots.name: slot_data, slot_weights.name: slot_weight, sequence_length.name: length, intent.name: intents, intent_2.name: intents_2}
#         print()
#         print(feed_dict)
#         print()
...
#         print('line: ', line)
        if data_processor.end == 1:
            ...
            def valid(in_path, slot_path, intent_path, intent_path_2):
                #바뀐 부분
                data_processor_valid = DataProcessor(in_path, slot_path, intent_path, intent_path_2, in_vocab, slot_vocab, intent_one_vocab, intent_two_vocab)
                ...
                while True:
                    in_data, slot_data, slot_weight, length, intents,intents_2, in_seq, slot_seq, intent_seq, intent_seq_2 = data_processor_valid.get_batch(arg.batch_size)
                    ...
                    for i in ret[2]:
                        pred_intents_2.append(np.argmax(i))
                    for i in intents_2:
                        correct_intents_2.append(i)
                    ...
                    if data_processor_valid.end == 1:
                        break
                #앞에 부분만 따졌을때 accuracy_1
                #뒷부분까지 accuracy
                pred_intents = np.array(pred_intents)
                correct_intents = np.array(correct_intents)
                pred_intents_2 = np.array(pred_intents_2)
                correct_intents_2 = np.array(correct_intents_2)
                accuracy_1 =(pred_intents==correct_intents) 
                accuracy = ((pred_intents==correct_intents) & (pred_intents_2 == correct_intents_2))
                semantic_error = accuracy
                
                accuracy_1 = accuracy_1.astype(float)
                accuracy_1 = np.mean(accuracy_1)*100.0
                ...
                logging.info('앞에거만 따졌을때: ' + str(accuracy_1))
                logging.info('모두 따졌을때 (2개 다): ' + str(accuracy))
            ...
            #바뀐 부분
            logging.info('Valid:')
            epoch_valid_slot, epoch_valid_intent, epoch_valid_err,valid_pred_intent,valid_correct_intent,valid_pred_slot,valid_correct_slot,valid_words,valid_gate = valid(os.path.join(full_valid_path, arg.input_file), os.path.join(full_valid_path, arg.slot_file), os.path.join(full_valid_path, arg.intent_file_one), os.path.join(full_valid_path, arg.intent_file_two))
            #바뀐 부분
            logging.info('Test:')
            epoch_test_slot, epoch_test_intent, epoch_test_err,test_pred_intent,test_correct_intent,test_pred_slot,test_correct_slot,test_words,test_gate = valid(os.path.join(full_test_path, arg.input_file), os.path.join(full_test_path, arg.slot_file), os.path.join(full_test_path, arg.intent_file_one), os.path.join(full_test_path,arg.intent_file_two))
