...
class DataProcessor(object):
    def __init__(self, in_path, slot_path, intent_path, intent_path_2,in_vocab, slot_vocab, intent_vocab, intent_vocab_2):
        self.__fd_in = open(in_path, 'r')
        self.__fd_slot = open(slot_path, 'r')
        self.__fd_intent = open(intent_path, 'r')
        self.__fd_intent_2 = open(intent_path_2, 'r')
        self.__in_vocab = in_vocab
        self.__slot_vocab = slot_vocab
        self.__intent_vocab = intent_vocab
        self.__intent_vocab_2 = intent_vocab_2
        self.end = 0
    ...
    def get_batch(self, batch_size):
        ...
        for i in range(batch_size):
            ...
            intent_2 = self.__fd_intent_2.readline()
            
            inp = inp.rstrip()
            slot = slot.rstrip()
            intent = intent.rstrip()
            intent_2 = intent_2.rstrip()
            
            in_seq.append(inp)
            slot_seq.append(slot)
            intent_seq.append(intent)
            intent_seq_2.append(intent_2)
...
#             print('intent2: ', intent_2)
        ...
        return in_data, slot_data, slot_weight, length, intents, intents_2, in_seq, slot_seq, intent_seq, intent_seq_2
